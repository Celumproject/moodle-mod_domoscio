...
if (!empty($id)) {
    ...
    if(!$scoid)
    {
      if (! $scoid = $DB->get_record("scorm_scoes", array("scorm" => $scorm->id, "scormtype" => "sco"))->id)
      {
          print_error('invalidcoursemodule');
      }
    }
...
domoscio_scorm_check_mode($scorm, $newattempt, $attempt, $USER->id, $mode);
...
$PAGE->set_url($url);
$PAGE->set_pagetype('mod-scorm-player');
...
if (empty($scorm->popup) || $displaymode == 'popup') {
    $exitlink = html_writer::link($CFG->wwwroot.'/mod/domoscio/view.php?id='.$domoscioid, $strexit, array('title' => $strexit));
